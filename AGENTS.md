...
Current release: **`v0.15.0`**. **Phase 13 (the GUI app) is in progress**: the **desktop client** (`apps/desktop/`, Tauri + React) is being built on a **Python-sidecar bridge** (`drift.sidecar`) that reuses the existing audited core — no new crypto. **Sub-phase 13a (shared Rust `drift-core`) is underway**: the `drift-core/` Cargo workspace's `drift-crypto` crate ports the protocol crypto and is proven **bit-for-bit against the shared `tests/vectors/`** exported from the Python reference (`scripts/export_vectors.py`). The full crypto surface is ported — base58, HKDF, the AEAD envelope, identity/Ed25519/ECDH, sealed sender, the full Double Ratchet, X3DH, burn tokens, the Argon2id vault, **stealth addressing, and FMD**. It composes vetted crates only; stealth + FMD bind **libsodium** (`libsodium-sys-stable`, vendored) for the exact ed25519 group ops (`crypto_core_ed25519_from_uniform` etc.), the same primitive the Python side reaches through PyNaCl. Remaining 13a: the FFI surface (13b). The **native mobile apps** remain design-only (`docs/app-plan.md`).
...
- **Phase 13 — GUI app** 🚧 in progress: native iOS (Swift) + Android (Kotlin) + desktop (Tauri). The end-state is a **shared Rust core** (`drift-core`) exposed via UniFFI, with the Python package as the reference impl and the source of cross-impl test vectors. **Desktop is being built first, via a deliberate bridge** (decided with the maintainer): a Tauri + React shell (`apps/desktop/`) over a **Python sidecar** (`drift.sidecar`, JSON-RPC on stdio) that wraps the *existing* `drift.storage` + `drift.transport.Session` — so the GUI ships on the proven crypto now and the sidecar is later swapped for native `drift-core` under the same UI, with no UI rewrite. This means the desktop GUI adds **no new cryptography** — `drift.sidecar` is a thin transport/marshalling layer, not a place for key handling. **13a (core extraction) is underway**: `drift-core/crypto` (the `drift-crypto` crate) ports the protocol crypto and is proven bit-for-bit against `tests/vectors/` (exported by `scripts/export_vectors.py`, asserted from both sides by `tests/unit/test_vectors.py` and `drift-core/crypto/tests/vectors.rs`, both wired into CI). It composes vetted crates only — the iron rule holds inside the Rust core too. Ported: base58, HKDF, AEAD, identity/Ed25519/ECDH, sealed sender, Double Ratchet, X3DH, burn, Argon2id vault, stealth, and FMD — the last two binding libsodium (`libsodium-sys-stable`) for the exact ed25519 group ops. The mobile apps remain design-only and still confront the mobile threat-model tensions head-on — the headline one being **no APNs/FCM push by default** (instant background push would leak timing metadata to Apple/Google), plus embedded Tor (Arti), platform keystore vault wrapping, and a mobile Lockdown/duress UX. Sub-phases 13a–13f (core extraction → FFI → desktop → Android → iOS → hardening); groups/rooms/beacon on the app are deferred to 13g+. Full plan: `docs/app-plan.md`. **Before starting the FFI surface or the mobile apps, get an explicit go-ahead and a call on the open decisions in §9 of that doc.**
- **Gauntlet** ✅ complete (`v0.15.0`, extended since): a standalone 11-probe adversarial test harness (`scripts/gauntlet.py`, also a CI job) that spins up the relay in-process and fires red-team probes at the core privacy/crypto invariants (relay blindness, stealth unlinkability, forward secrecy, burn replay, sealed-sender opacity, WITNESS chain integrity, forged-message rejection, panic isolation, OTPK consumption, FMD rate, PQ downgrade resistance).
- **Hybrid post-quantum handshake (PQXDH-style)** ✅ complete: the X3DH
  bootstrap is hybrid by default — bundles publish a signed **ML-KEM-768**
  prekey (FIPS 203, via `cryptography`/OpenSSL — no new dependency), the sender
  encapsulates, and the KEM secret joins the master-secret KDF
  (`info="drift-pqxdh-v1"`). Defeats harvest-now-decrypt-later for the
  handshake; never weaker than classic X3DH; strip attempts hard-fail
  (gauntlet probe `pq-downgrade`); pre-PQ peers downgrade *visibly*. Only the
  handshake is hybrid — the ratchet stays X25519 (DESIGN.md §4, PROTOCOL.md §5;
  `crypto/pqkem.py`, `crypto/x3dh.py`, vector `tests/vectors/pqxdh.json`).
