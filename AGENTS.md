...
Current release: **`v0.15.0`**. **Phase 13 (the GUI app) is in progress**: the **desktop client** (`apps/desktop/`, Tauri + React) is being built on a **Python-sidecar bridge** (`drift.sidecar`) that reuses the existing audited core — no new crypto. **Sub-phase 13a (shared Rust `drift-core`) has begun**: the `drift-core/` Cargo workspace's `drift-crypto` crate ports the core protocol crypto and is proven **bit-for-bit against the shared `tests/vectors/`** exported from the Python reference (`scripts/export_vectors.py`). Ported so far: base58, HKDF, the AEAD envelope, identity/Ed25519/ECDH, sealed sender, the full Double Ratchet, X3DH, burn tokens, and the Argon2id vault. Still to port: **stealth addressing + FMD** (need libsodium's exact Elligator `crypto_core_ed25519_from_uniform`). The **native mobile apps** remain design-only (`docs/app-plan.md`).
