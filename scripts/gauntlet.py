...
def main() -> int:
    ...
    probes: list[tuple[str, ProbeFn]] = [
        ("relay cannot enumerate contacts", g.probe_relay_blindness),
        ("10 messages produce 10 unlinkable addresses", g.probe_stealth_unlinkability),
        ("deleted message keys cannot decrypt past ciphertext", g.probe_forward_secrecy),
        ("burn token cannot be replayed", g.probe_burn_replay),
        ("relay stored blobs contain no sender identity", g.probe_sealed_sender),
        ("WITNESS chain is signed and hash-linked", g.probe_witness_chain),
        ("tampered ciphertext raises InvalidTag, ratchet state survives", g.probe_forged_message),
        ("duress passphrase produces unreachable real identity", g.probe_panic_isolation),
        ("consumed OTPK is deleted and non-reusable", g.probe_otpk_consumption),
        ("FMD rate dial controls relay-side detection probability", g.probe_fmd_rate),
    ]
    ...
    for name, fn in probes:
        ...
        try:
            ok, detail = fn()
        except Exception as exc:  # noqa: BLE001 — a probe crash is a FAIL, not a stop
            ok, detail = False, f"{type(exc).__name__}: {exc}"
        elapsed_ms = (time.perf_counter() - started) * 1000.0
        if ok:
            passed += 1
            console.print(
                f"[bold green]✓  PASS[/bold green]  {name}  "
                f"[dim]{detail} · {elapsed_ms:.0f}ms[/dim]"
            )
        else:
            console.print(
                f"[bold red]✗  FAIL[/bold red]  {name}  "
                f"[dim]{detail} · {elapsed_ms:.0f}ms[/dim]"
            )

    _summary(passed, len(probes))
